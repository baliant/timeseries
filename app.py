...
def calc_pid_metrics(df, pv, sp, out, band=None):
    ...
    itae = float(
        (
            tmp["AbsError"]
            * (tmp["Timestamp"] - tmp["Timestamp"].iloc[0]).dt.total_seconds()
            * tmp["dt_s"]
        ).sum()
    )
...
def estimate_pi_from_data(tmp, pv, sp, out, pv_span=None, out_span=100.0, min_error_change=1e-9):
    ...
    if pv_span is None or pv_span == 0:
        pv_span = np.nanpercentile(np.r_[tmp[pv].to_numpy(), tmp[sp].to_numpy()], 95) - np.nanpercentile(
            np.r_[tmp[pv].to_numpy(), tmp[sp].to_numpy()], 5
        )
...
def build_overlay_multi_axis_figure(df, selected, normalize, axis_map):
    fig = go.Figure()
    axis_groups = sorted({axis_map[c] for c in selected})
    axis_side_cycle = {1: "left", 2: "right", 3: "left", 4: "right"}
    axis_positions = {1: 0.0, 2: 1.0, 3: 0.06, 4: 0.94}
    ...
    for axis_num in axis_groups:
        ...
        group_signals = [c for c in selected if axis_map[c] == axis_num]
        title = " / ".join(group_signals[:3])
        if len(group_signals) > 3:
            title += " ..."
        axis_cfg = {
            "title": "z-score" if normalize else title,
            "side": axis_side_cycle.get(axis_num, "left"),
            "showgrid": axis_num == 1,
            "zeroline": False,
        }
        if axis_num != 1:
            axis_cfg.update({"overlaying": "y", "anchor": "free", "position": axis_positions.get(axis_num, 1.0)})
        layout[key] = axis_cfg
...
def build_stacked_figure(df, selected, normalize):
    ...
    for idx, c in enumerate(selected, start=1):
        ...
        if normalize:
            sd = y.std()
            y = (y - y.mean()) / sd if sd and np.isfinite(sd) else y * 0
        fig.add_trace(go.Scattergl(x=df["Timestamp"], y=y, mode="lines", name=c), row=idx, col=1)
        fig.update_yaxes(title_text="z-score" if normalize else c, row=idx, col=1)
    fig.update_layout(height=max(350, 220 * len(selected)), hovermode="x unified", legend=dict(orientation="h"))
    fig.update_xaxes(title_text="Time", row=len(selected), col=1)
    return fig


def build_correlation_heatmap(corr):
    ...
    fig.update_layout(height=max(380, 60 + 35 * len(corr.columns)), xaxis_title="Signal", yaxis_title="Signal")
    return fig
...
if uploaded is not None:
    ...
    local_candidates = [
        Path("PG_21_keszulek_07-20-07-22.xlsx"),
        Path("/mnt/data/PG_21_keszulek_07-20-07-22.xlsx"),
    ]
...
if np.isfinite(std_dt) and np.isfinite(median_dt) and std_dt > max(1.0, 0.05 * median_dt):
...
with tab1:
    ...
    if selected:
        if trend_mode == "Shared y-axis":
            fig = go.Figure()
            for c in selected:
                y = pd.to_numeric(filtered[c], errors="coerce")
                if normalize:
                    sd = y.std()
                    y = (y - y.mean()) / sd if sd and np.isfinite(sd) else y * 0
        ...
        elif trend_mode == "Separate overlay y-axes":
            max_groups = min(4, max(1, len(selected)))
            st.caption("Assign each selected signal to an axis group. Signals in the same group share the same y-axis.")
            axis_cols = st.columns(min(4, len(selected)))
            axis_map = {}
            for idx, c in enumerate(selected):
                with axis_cols[idx % len(axis_cols)]:
                    axis_map[c] = st.selectbox(
                        f"Axis for {c}",
                        options=list(range(1, max_groups + 1)),
                        index=min(idx, max_groups - 1),
                        key=f"axis_{c}",
                    )
            fig = build_overlay_multi_axis_figure(filtered, selected, normalize, axis_map)
            st.plotly_chart(fig, use_container_width=True)

        else:
            fig = build_stacked_figure(filtered, selected, normalize)
            st.plotly_chart(fig, use_container_width=True)
...
with tab3:
    ...
    if sp == "<none>":
        ...
        band = st.number_input(
            "Acceptable absolute PV error band (engineering units)",
            min_value=0.0,
        ...
        st.write(
            f"For a sustained **{err_pct:g}%** error: proportional action = **{p_action:g}% OUT immediately**, "
            f"integral ramp = **{i_rate:g}% OUT/s** = **{i_rate*60:g}% OUT/min**. "
            f"After one Ti ({ti:g} s), the integral contribution equals **{p_action:g}% OUT**."
        )

        with st.expander("Experimental: estimate PI parameters from PV/SP/OUT data"):
            st.warning(
                "Use this only as a diagnostic. A closed-loop trend generally cannot identify tuning reliably unless "
                "signal scaling, controller action, output limits, filtering, and disturbances are understood."
            )
...
st.divider()
st.caption(
    "Engineering note: controller performance metrics are only meaningful when PV, SP and OUT are correctly assigned and scaled. "
    "Saturation, cascade/RCAS mode, split-range logic, filtering, bad-status periods and manual mode can invalidate tuning conclusions."
)
